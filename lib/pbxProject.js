...
pbxProject.prototype.addAppExtensions = function(fpath, opt) {
    var embeddedFile = new pbxFile(fpath, opt);
    embeddedFile.uuid = this.generateUuid();
    embeddedFile.fileRef = this.generateUuid();
    embeddedFile.target = opt ? opt.target : undefined;

    if (this.hasFile(embeddedFile.path)) return false;

        //keeping a separate PBXBuildFile entry for Embed Frameworks
    this.addToPbxBuildFileSection(embeddedFile);        // PBXBuildFile
    this.addToPbxFileReferenceSection(embeddedFile);    // PBXFileReference
    this.addToFrameworksPbxGroup(embeddedFile);                 // PBXGroup
    this.addToPbxEmbedAppExtensionsBuildPhase(embeddedFile); // PBXCopyFilesBuildPhase
...
pbxProject.prototype.addToPbxEmbedAppExtensionsBuildPhase = function (file) {
    var sources = this.pbxEmbedAppExtensionsBuildPhaseObj(file.target);
    if (sources) {
        sources.files.push(pbxBuildPhaseObj(file));
    }
}

pbxProject.prototype.removeFromPbxEmbedAppExtensionsBuildPhase = function (file) {
    var sources = this.pbxEmbedAppExtensionsBuildPhaseObj(file.target);
    if (sources) {
        var files = [];
        for (i in sources.files) {
...
pbxProject.prototype.pbxEmbedAppExtensionsBuildPhaseObj = function (target) {
    return this.buildPhaseObject('PBXCopyFilesBuildPhase', 'Embed App Extensions', target);
};
...
pbxProject.prototype.addToLibrarySearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if (typeof file === 'string') {
            ...
            let willAddPath = searchPathForFile(file, this);
            let willAdd = true;
            for(let p of buildSettings['LIBRARY_SEARCH_PATHS']){
                if(p == willAddPath){
                    willAdd = false;
                }
            }
            if(willAdd){
                buildSettings['LIBRARY_SEARCH_PATHS'].push(willAddPath);
            }
